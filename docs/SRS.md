...
| ID | Path | Requirement |
|---|---|---|
| FR-201 | `/admin/login` | Email + password, then **TOTP 2FA (mandatory for every user)**. Enrollment forced on first login; 10 single-use recovery codes. Generic error text. Password reset by email. As built (Phase 18): enrolment shows the setup key and an `otpauth:` link (no QR code yet); `/admin/account` regenerates recovery codes and changes the password, signing out other sessions. |
...
| FR-204 | `/admin/startups/new`, `/admin/startups/[id]/edit` | Sectioned form: basics, media, classification, location, founders, investors, batches, rounds. Comboboxes link existing records or create drafts inline. Save-draft and Publish are distinct actions. As built (Phase 18): the edit page is `/admin/startups/[id]` (every entity likewise). Field errors come from the server's validation; the form pre-checks only shape and required fields, because the Zod schemas stay server-side (SEC-01). Publishing a startup can also publish the drafts it links to, on by default. |
| FR-205 | `/admin/categories` | Edit `taxonomy_pages` copy for existing facet values only. |
| FR-206 | `/admin/import` | Upload CSV → dry-run table → commit within 24 h. As built (Phase 18): `?job=` reopens a stored dry run from the dashboard. |
...
| SEC-06 | **Uploads.** MIME sniffed from magic bytes; allowlist jpg/png/webp/**avif** (AVIF added 2026-09-17: many company sites serve logos only in it; accepted only when the `ftyp` brands name AVIF and the decoded header reports AV1, so HEIC and other HEIF files stay refused, under the same byte and pixel limits) — **SVG refused** (tightened 2026-09-16: every upload is rasterised anyway, so accepting SVG would add an XML parser, historically the most exploited path in image libraries, for no product gain; editors export PNG and prefill falls back to a site's raster icon); 5 MB byte cap; sharp `limitInputPixels` = 24 MP (decompression-bomb guard) and `failOn: 'error'`; SVG rasterized at a capped density with external references disallowed; output dimensions capped; re-encoded to WebP (strips EXIF and embedded payloads); random Blob prefixes. | Unit tests |
