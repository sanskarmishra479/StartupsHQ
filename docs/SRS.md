...
| SEC-09 | **Headers & CSP.** Admin origin: nonce-based strict CSP (`script-src 'nonce-…' 'strict-dynamic'`), rendered dynamically. Public origin: hash-based CSP via Next.js `experimental.sri` so pages stay statically cacheable — **spike resolved 2026-09-16: it works.** `experimental.sri: { algorithm: 'sha256' }` stamps `integrity` on every script Next.js emits and the build still prerenders pages as static, so the public policy is `script-src 'self'` with no inline scripts and no per-request nonce. Built in `src/lib/security-headers.ts`, applied by `src/proxy.ts`, which is the only place that knows which host answered. Inline **styles** are allowed on both origins, because Next.js inlines the stylesheet and a nonce cannot cover a cached page; scripts, the dangerous half, are not. If SRI had proved unworkable, the fallback was a CSP without script nonces that still enforces `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, `frame-ancestors 'none'`, `img-src 'self' <blob host>` — acceptable because no credential is valid on the public origin (ADR-014). Both origins: HSTS `max-age=31536000; includeSubDomains` (**preload only post-launch**, SEC-19), `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `X-Frame-Options: DENY`, minimal Permissions-Policy. | Header assertion tests |
| SEC-10 | **DB roles.** Migrations create NOLOGIN group roles and default privileges; each environment creates its own LOGIN users (passwords never in git) and grants membership. `startupshq_app` (app_rw): DML on content tables, INSERT/SELECT only on `audit_log` and `erasure_log`, no DDL. `migrator`: DDL, used **only** by the GitHub Actions migration workflow in a protected environment — never present in Vercel. `startupshq_retention`: SELECT/UPDATE/DELETE on `audit_log` only. `startupshq_backup`: read-only. TLS via the Neon pooler. | Test: DDL as `app_rw` → permission denied |
