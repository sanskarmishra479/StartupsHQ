...
| [ARCHITECTURE.md](./ARCHITECTURE.md) | **How is it put together, and why?** | Layering, lifecycles, caching, ops, cost model, Decision Records (ADR-001 … ADR-023) | Before changing anything structural |
...
| 2026-09-17 | **SEC-06 widened: AVIF accepted** for uploads and prefill (owner's decision), only when the file is really AV1; SVG, ICO and HEIC stay refused. API §8.5–8.6 record it and prefill's candidate order: several logo and cover URLs tried in turn, and storage failures reported as such. TEST_PLAN SEC-06 lists the new cases |
| 2026-09-17 | Phase 18: admin UI built — sign-in with TOTP, enrolment and recovery codes, dashboard, entity lists and editors, the startup form with inline drafts, prefill, uploads and rounds, import, media, categories, users, privacy and account. **ADR-024** records how the admin nonce reaches Next.js and the theme script's hash; SRS SEC-09, FR-201, FR-204 and FR-206 record what was built; TEST_PLAN names `admin-crud.spec.ts`'s environment |
...
| 2026-09-16 | Phase 13: **frontend direction recorded** — PRD §3 and §7 take phantom.land's feel for the front door; SRS FR-113 makes `/` the immersive landing and FR-101 moves the explore grid to `/companies`; ADR-023 records the landing as progressive enhancement (OGL, atlas, barrel pass). TEST_PLAN adds FR-113 and the token contrast test |
| 2026-09-16 | Phase 13: **SEC-09 corrected (ADR-022)** — the public CSP without `'unsafe-inline'` blocked the inline scripts carrying each page's React payload, so nothing hydrated; the public origin now runs ADR-014's fallback, `script-src 'self' 'unsafe-inline'`, keeping every other directive. TEST_PLAN SEC-09 now requires a hydrated page with no CSP violation in the console |
| 2026-09-16 | **Phase 12 complete — backend done (M5).** SEC-10, SEC-12 and SEC-13 now have executable verification (role tests that connect as each role; an error-shape audit; a supply-chain guard). Scheduled jobs for audit retention and the ECB rates join media GC in `maintenance.yml`, each gated to its own schedule. WAF rules written down in ARCHITECTURE §8 for Phase 22. API §6–§8 verified against the handlers; **v1 frozen** |
| 2026-09-16 | Phase 12: SEC-09 spike resolved — `experimental.sri` keeps public pages static while stamping script integrity, so the public CSP needs no nonce; the admin origin gets a fresh nonce per request. Both origins now carry HSTS without `preload` (SEC-19) |
| 2026-09-16 | Phase 11: API §8.7 records the importer as built — the column set, unknown columns ignored, `update` only via an explicit slug, what a committed row creates, and the refusal codes. SRS FR-402 records measured trigram behaviour: 0.85 catches case, spacing and word-order variants but not `Inc`/`Ltd` suffixes or a dropped letter. **Backend feature work complete**; Phase 12 verifies it |
| 2026-09-16 | Prefill checked against real sites (the manual step in TEST_PLAN §13), which found two things the fixtures could not. **SEC-05 corrected:** IPv4 embedded in IPv6 is now judged by the address it embeds, so the NAT64 prefix resolvers synthesise (`64:ff9b::/96`) no longer refuses real hosts, and the socket is handed only the validated addresses of an answer instead of the whole answer being refused when one record is non-public. **FR-401 corrected:** the careers link prefers the company's own host or a hiring platform, after prefill picked an acquirer's careers page |
| 2026-09-16 | Phase 10: API §8.6 records prefill as built — `20`/hour per editor failing closed, a rejected page URL as `400 UNSAFE_URL` (plain `http` included) while a rejected image only warns, `source` and `confidence` values, and when Firecrawl is consulted. SRS FR-401 and TEST_PLAN SEC-05 name the real test files |
| 2026-09-16 | Phase 9: **SVG uploads refused** — SEC-06 tightened, since every upload is rasterised and accepting SVG would add an XML parser for no gain; API §8.5 rewritten as built (magic-byte sniffing, header-based 24 MP check, no upscaling, `og` not an accepted purpose). FR-408 and FR-111 record the pipeline, the share-card refresh and the GC; TEST_PLAN SEC-06 lists the real fixtures |
| 2026-09-16 | Domain bought: **startupshq.space** (admin on `admin.startupshq.space`). Replaces the `startupshq.com` placeholder in SRS §2, API §1 and ARCHITECTURE; "domain TBD" is resolved |
