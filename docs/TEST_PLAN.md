...
| `prefill` | a page with JSON-LD, OpenGraph, icons and social links yields name, tagline, description, careers URL, links and a matched location; images become staging assets with `source_url`; a hostile `og:image` or Firecrawl image URL is skipped with a warning and no resolved address in it; a non-raster icon skipped; 403/500 pages degrade to warnings; Firecrawl used only when the page gave nothing, absent key degrades; no entity is ever persisted; 20 per hour then `429`, and a limiter outage refuses (fails closed) |
