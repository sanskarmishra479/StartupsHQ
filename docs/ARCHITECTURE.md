...
### ADR-023 — Immersive landing as progressive enhancement, beside a plain full list
**Status:** Accepted · 2026-09-16

**Context.** The owner wants startups.gallery's content with the feel of phantom.land: a dark, curved, endlessly draggable grid. A canvas-only page would fail NFR-03 (crawlable links), NFR-04 (keyboard, screen readers, reduced motion) and the ≥ 95 Lighthouse target if it blocked on a loader. The full, filterable directory still has to exist for people who want to search rather than wander.
**Decision.** `/` is the immersive landing (FR-113): the most recent ≤ 300 published cards, server-rendered as a flat grid of links, upgraded after load to a WebGL canvas when motion is allowed and WebGL works. The FR-101 explore grid moves to `/companies` with the same cards and tokens. Rendering: OGL (pinned, bundled for the CSP, dynamic-imported), one instanced draw per frame from a texture atlas that cards are rasterised into when they first come into view, and a barrel post-pass whose formula is shared with hit-testing (`src/components/landing/grid-math.ts`). The canvas redraws only while something moves. Cards are landscape at the OpenGraph ratio 1.91:1, since covers are overwhelmingly OG images. The landing's data is one additive cached read, not a larger public API page (SEC-15 unchanged).
